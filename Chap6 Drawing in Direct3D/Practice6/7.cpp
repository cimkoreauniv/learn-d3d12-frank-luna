...
private:
    ...
    XMMATRIX mWorld_pyr = XMMatrixIdentity();
    XMMATRIX mWorld_box = XMMatrixTranslation(2.0f, 0.0f, 0.0f);
...
void BoxApp::Update(const GameTimer& gt)
{
    ...
    XMStoreFloat4x4(&objConstants[0].WorldViewProj, XMMatrixTranspose(mWorld_pyr * mView * mProj));
    XMStoreFloat4x4(&objConstants[1].WorldViewProj, XMMatrixTranspose(mWorld_box * mView * mProj));
    mObjectCB->CopyData(0, objConstants[0]);
    mObjectCB->CopyData(1, objConstants[1]);
}
...
void BoxApp::Draw(const GameTimer& gt)
{
    ...
    mCommandList->DrawIndexedInstanced(mBoxGeo->DrawArgs["Box"].IndexCount, 1, 18, 5, 0);
    mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(),
...
void BoxApp::BuildConstantBuffers()
{
    ...
    cbvDesc.SizeInBytes = objCBByteSize * 2;
...
{
    std::array<Vertex, 5> vertices_pyr =
    {
        Vertex({ XMFLOAT3(-1.0f, -1.0f, +1.0f), XMFLOAT4(Colors::Green) }),
        Vertex({ XMFLOAT3(+1.0f, -1.0f, +1.0f), XMFLOAT4(Colors::Green) }),
        Vertex({ XMFLOAT3(-1.0f, -1.0f, +3.0f), XMFLOAT4(Colors::Green) }),
        Vertex({ XMFLOAT3(+1.0f, -1.0f, +3.0f), XMFLOAT4(Colors::Green) }),
        Vertex({ XMFLOAT3(+0.0f, +2.0f, +2.0f), XMFLOAT4(Colors::Red) })
    };

    std::array<std::uint16_t, 18> indices_pyr =
    {
        0, 1, 3,
        0, 3, 2,
    ...
    std::array<Vertex, 8> vertices_box =
    {
    ...
    std::array<std::uint16_t, 36> indices_box =
    {
        // front face
        0, 1, 2,
        0, 2, 3,
    ...
    for (int i = 0; i < 8; i++)
        vertices[5 + i] = vertices_box[i];
    for (int i = 0; i < 18; i++)
        indices[i] = indices_pyr[i];
    for (int i = 0; i < 36; i++)
        indices[18 + i] = indices_box[i];
    ...
    SubmeshGeometry submesh_pyr, submesh_box;
    submesh_pyr.IndexCount = 18U;
    submesh_pyr.StartIndexLocation = 0;
    submesh_pyr.BaseVertexLocation = 0;

    submesh_box.IndexCount = 36;
    submesh_box.StartIndexLocation = 18;
    submesh_box.BaseVertexLocation = 5;
    mBoxGeo->DrawArgs["Pyramid"] = submesh_pyr;
