...
private:
	...
	std::vector<D3D12_INPUT_ELEMENT_DESC> mSquareInputLayout;
	...
	SubmeshGeometry mSquareSub[gNumSquares + 1];
...
bool BlendDemoApp::Initialize()
{
	...
	FlushCommandQueue();
	OutputDebugString(std::to_wstring(sizeof(PassConstants)).c_str());
	return true;
}
...
void BlendDemoApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->SetPipelineState(mPSOs["squareDraw"].Get());
	mCommandList->IASetVertexBuffers(0, 1, &mGeometries["squaresGeo"]->VertexBufferView());
	mCommandList->IASetIndexBuffer(&mGeometries["squaresGeo"]->IndexBufferView());
	for (int i = 0; i < gNumSquares; i++)
	{
		mCommandList->OMSetStencilRef(i);
		SubmeshGeometry sub = mSquareSub[i];
...
void BlendDemoApp::BuildShadersAndInputLayout()
{
	...
	mShaders["squareVS"] = d3dUtil::CompileShader(L"Shaders/color.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["squarePS"] = d3dUtil::CompileShader(L"Shaders/color.hlsl", nullptr, "PS", "ps_5_0");
}

void BlendDemoApp::BuildLandGeometry()
{
...
void BlendDemoApp::BuildWavesGeometry()
{
...
void BlendDemoApp::BuildBoxGeometry()
{
	using GG = GeometryGenerator;
	GG::MeshData box = GG::CreateBox(8.0f, 8.0f, 8.0f, 3);
...
void BlendDemoApp::BuildPSOs()
{
	...
	D3D12_DEPTH_STENCIL_DESC markDSS = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT);
	//markDSS.DepthEnable = false;
	markDSS.StencilEnable = true;
	markDSS.StencilReadMask = 0xff;
	markDSS.StencilWriteMask = 0xff;
	markDSS.FrontFace.StencilPassOp = D3D12_STENCIL_OP_INCR;
	markDSS.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_INCR;
	markDSS.FrontFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	markDSS.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;

	markDSS.BackFace.StencilPassOp = D3D12_STENCIL_OP_INCR;
	markDSS.BackFace.StencilDepthFailOp = D3D12_STENCIL_OP_INCR;
	markDSS.BackFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	markDSS.BackFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;

	opaquePsoDesc.RasterizerState = CD3DX12_RASTERIZER_DESC(D3D12_DEFAULT);
	opaquePsoDesc.BlendState = CD3DX12_BLEND_DESC(D3D12_DEFAULT);
	opaquePsoDesc.DepthStencilState = markDSS;
	...
	auto squareDrawPSODesc = opaquePsoDesc;
	auto squareDSS = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT);
	squareDSS.DepthEnable = false;
	squareDSS.StencilEnable = true;
	squareDSS.StencilReadMask = 0xff;
	squareDSS.StencilWriteMask = 0;

	squareDSS.FrontFace.StencilPassOp = D3D12_STENCIL_OP_KEEP;
	squareDSS.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
	squareDSS.FrontFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	squareDSS.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;

	squareDSS.BackFace.StencilPassOp = D3D12_STENCIL_OP_KEEP;
	squareDSS.BackFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
	squareDSS.BackFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
	squareDSS.BackFace.StencilFunc = D3D12_COMPARISON_FUNC_NEVER;

	squareDrawPSODesc.DepthStencilState = squareDSS;
	squareDrawPSODesc.InputLayout = { mSquareInputLayout.data(), (UINT)mSquareInputLayout.size() };
	squareDrawPSODesc.VS =
	{
		reinterpret_cast<BYTE*>(mShaders["squareVS"]->GetBufferPointer()),
		mShaders["squareVS"]->GetBufferSize()
	};
	squareDrawPSODesc.PS =
	{
		reinterpret_cast<BYTE*>(mShaders["squarePS"]->GetBufferPointer()),
		mShaders["squarePS"]->GetBufferSize()
	};
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&squareDrawPSODesc, IID_PPV_ARGS(&mPSOs["squareDraw"])));
}
...
void BlendDemoApp::BuildRenderItems()
{
	...
	OutputDebugString((std::to_wstring(mRitemLayer[0].size()) + L"\n").c_str());
...
void BlendDemoApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	OutputDebugString((std::to_wstring(ritems.size()) + L"\n").c_str());
...
void BlendDemoApp::BuildSquaresGeometry()
{
	std::vector<VertexC> vertices(4 * gNumSquares);
	XMFLOAT4 colors[gNumSquares] = {
		XMFLOAT4(),
	...
	auto mSquaresGeo = std::make_unique<MeshGeometry>();
	mSquaresGeo->Name = "boxGeo";
	...
	mSquaresGeo->VertexByteStride = sizeof(VertexC);
	...
	for (int i = 0; i < gNumSquares; i++)
	{
		...
		submesh.StartIndexLocation = 0;
		submesh.BaseVertexLocation = 4 * i;

		mSquareSub[i] = submesh;
	}
	mGeometries["squaresGeo"] = std::move(mSquaresGeo);
}
