...
void BlendDemoApp::AnimateMaterials(const GameTimer& gt)
{
	...
	auto lightningMat = mMaterials["lightning"].get();
	int idx = (int)(gt.TotalTime() * 60.0f) % 60;
	lightningMat->DiffuseSrvHeapIndex = idx + 2;
}
...
void BlendDemoApp::LoadTextures()
{
	for(int i=1; i<=60; i++)
	{
		...
		fenceTex->Name += std::to_string(i);
		fenceTex->Filename = L"../../ExerciseMedia/BoltAnim/Bolt";
		if (i < 10)
			fenceTex->Filename += L"0";
		fenceTex->Filename += L"0" + std::to_wstring(i) + L".dds";
...
void BlendDemoApp::BuildDescriptorHeaps()
{
	...
	for (int i = 1; i <= 60; i++)
	{
		std::string texName = "fenceTex";
		texName += std::to_string(i);
		auto fenceTex = mTextures[texName]->Resource;
		hDescriptor.Offset(1, mCbvSrvUavDescriptorSize);
...
void BlendDemoApp::BuildCylGeometry()
{
	using GG = GeometryGenerator;
	GG::MeshData box = GG::CreateCaplessCylinder(4.0f, 4.0f, 8.0f, 20, 5);
...
void BlendDemoApp::BuildPSOs()
{
	...
	D3D12_RENDER_TARGET_BLEND_DESC alphaBlendDesc = transparencyBlendDesc;
	alphaBlendDesc.SrcBlend = D3D12_BLEND_SRC_ALPHA;
	alphaBlendDesc.DestBlend = D3D12_BLEND_ONE;
	alphaTestedPsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;
	alphaTestedPsoDesc.BlendState.RenderTarget[0] = alphaBlendDesc;
...
void BlendDemoApp::BuildMaterials()
{
	...
	lightning->DiffuseSrvHeapIndex = 2;
...
void BlendDemoApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&cylRitem->World, XMMatrixTranslation(3.0f, 5.0f, -9.0f));
	cylRitem->ObjCBIndex = 2;
	cylRitem->Mat = mMaterials["lightning"].get();
	cylRitem->Geo = mGeometries["boxGeo"].get();
	cylRitem->IndexCount = cylRitem->Geo->DrawArgs["box"].IndexCount;
	cylRitem->StartIndexLocation = cylRitem->Geo->DrawArgs["box"].StartIndexLocation;
	cylRitem->BaseVertexLocation = cylRitem->Geo->DrawArgs["box"].BaseVertexLocation;

	mRitemLayer[(int)RenderLayer::AlphaTested].push_back(cylRitem.get());
