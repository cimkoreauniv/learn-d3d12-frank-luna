...
public:
    ...
    LitWavesApp& operator=(const LitWavesApp& other) = delete;
    ~LitWavesApp();
...
void LitWavesApp::OnKeyboardInput(const GameTimer& gt)
{
    ...
    if (GetAsyncKeyState('A') & 0x8000)
        mMaterials["water"]->Roughness -= 0.5f * dt, mMaterials["water"]->NumFramesDirty = gNumFrameResources;
    if (GetAsyncKeyState('D') & 0x8000)
        mMaterials["water"]->Roughness += 0.5f * dt, mMaterials["water"]->NumFramesDirty = gNumFrameResources;
    if (GetAsyncKeyState('S') & 0x8000)
        mMaterials["grass"]->Roughness -= 0.5f * dt, mMaterials["grass"]->NumFramesDirty = gNumFrameResources;
    if (GetAsyncKeyState('W') & 0x8000)
        mMaterials["grass"]->Roughness += 0.5f * dt, mMaterials["grass"]->NumFramesDirty = gNumFrameResources;
    mMaterials["water"]->Roughness = MathHelper::Clamp(mMaterials["water"]->Roughness, 0.0f, 1.0f);
    mMaterials["grass"]->Roughness = MathHelper::Clamp(mMaterials["grass"]->Roughness, 0.0f, 1.0f);
