...
LitColumnsApp::LitColumnsApp(HINSTANCE hInstance) : D3DApp(hInstance) {
    mMainWndCaption = L"LitColumns App";
}
...
void LitColumnsApp::Draw(const GameTimer& gt)
{
    ...
    ThrowIfFailed(mCommandList->Reset(cmdListAlloc.Get(), mIsWireFrame ? mPSOs["opaque_wireframe"].Get() : mPSOs["opaque"].Get()));
...
void LitColumnsApp::BuildRenderItems()
{
    ...
    circleRItem->mat = mMaterials["bricks0"].get();
...
{
    mShaders["standardVS"] = d3dUtil::CompileShader(L"Shaders/2.hlsl", nullptr, "VS", "vs_5_1");
    mShaders["opaqueGS"] = d3dUtil::CompileShader(L"Shaders/2.hlsl", nullptr, "GS", "gs_5_1");
    mShaders["opaquePS"] = d3dUtil::CompileShader(L"Shaders/2.hlsl", nullptr, "PS", "ps_5_1");
    mInputLayout =
    {
...
void LitColumnsApp::BuildPSOs()
{
    ...
    opaquePSODesc.GS = {
        mShaders["opaqueGS"]->GetBufferPointer(),
...
void LitColumnsApp::BuildCircleGeometry()
{
    using GG = GeometryGenerator;
    GG::MeshData meshData = GG::CreateGeosphere(1.0f, 0);
