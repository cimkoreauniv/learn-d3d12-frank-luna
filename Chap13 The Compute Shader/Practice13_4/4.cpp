...
bool BlurApp::Initialize()
{
	...
	mBlurFilter = std::make_unique<BlurFilter>(md3dDevice.Get(), mClientWidth, mClientHeight, mBackBufferFormat, 2.5f);
...
void BlurApp::Draw(const GameTimer& gt)
{
	...
	mBlurFilter->Execute(mCommandList.Get(), mPostProcessRootSignature.Get(),
		mPSOs["horzBlur"].Get(), mPSOs["vertBlur"].Get(), CurrentBackBuffer(), 3);
...
void BlurApp::BuildDescriptorHeaps()
{
	...
	ThrowIfFailed(md3dDevice->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(mCbvSrvUavDescriptorHeap.GetAddressOf())));
	...
	mBlurFilter->BuildDescriptors(
		CD3DX12_CPU_DESCRIPTOR_HANDLE(
			mCbvSrvUavDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), 3, mCbvSrvUavDescriptorSize),
		CD3DX12_GPU_DESCRIPTOR_HANDLE(
			mCbvSrvUavDescriptorHeap->GetGPUDescriptorHandleForHeapStart(), 3, mCbvSrvUavDescriptorSize),
...
void BlurApp::BuildShadersAndInputLayout()
{
	...
	mShaders["horzBlurCS"] = d3dUtil::CompileShader(L"Shaders/4.hlsl", nullptr, "HorzBlurCS", "cs_5_0");
	mShaders["vertBlurCS"] = d3dUtil::CompileShader(L"Shaders/4.hlsl", nullptr, "VertBlurCS", "cs_5_0");
