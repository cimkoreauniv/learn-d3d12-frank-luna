...
void InstancingAndCullingApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	...
	OutputDebugString((std::to_wstring(x) + L" " + std::to_wstring(y) + L"\n").c_str());
}
...
void InstancingAndCullingApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	invView = XMMatrixInverse(&XMMatrixDeterminant(mCamera.GetView()), mCamera.GetView());
}
...
void InstancingAndCullingApp::UpdateInstanceData(const GameTimer& gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		for (UINT i = 0; i < (UINT)instanceData.size(); ++i)
		{
			...
			XMMATRIX invWorld = e->invWorlds[i];
...
void InstancingAndCullingApp::BuildFrameResources()
{
	...
	for (int i = 0; i < gNumFrameResources; ++i)
	{
		mFrameResources.push_back(std::make_unique<FrameResource>(md3dDevice.Get(),
			1, maxInstanceCount, (UINT)mMaterials.size()));
	}
}
...
void InstancingAndCullingApp::BuildRenderItems()
{
	...
	skullRitem->invWorlds.resize(n * n * n);
	maxInstanceCount = n * n * n;
	...
	for (int k = 0; k < n; ++k)
	{
		for (int i = 0; i < n; ++i)
		{
			for (int j = 0; j < n; ++j)
			{
				...
				InstanceData& instance = skullRitem->Instances[index];
				...
				skullRitem->invWorlds[index] = XMMatrixInverse(&XMMatrixDeterminant(world), world);

				XMStoreFloat4x4(&instance.TexTransform, XMMatrixScaling(2.0f, 2.0f, 1.0f));
				instance.MaterialIndex = index % mMaterials.size();
			}
		}
	}
