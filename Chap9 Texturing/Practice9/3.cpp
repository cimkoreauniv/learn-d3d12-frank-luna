...
{
    XMMATRIX matTf = XMMatrixTranslation(-0.5f, -.5f, 0.f) * XMMatrixScaling(sqrt2, sqrt2, 1.0f)
        * XMMatrixRotationZ(gt.TotalTime()) * XMMatrixTranslation(.5f, .5f, .0f);
    XMStoreFloat4x4(&mMaterials["woodCrate"]->MatTransform, matTf);
...
void CrateApp::LoadTextures()
{
    ...
    flareTex->Filename = L"../../ExerciseMedia/flare.dds";
    CreateDDSTextureFromFile12(md3dDevice.Get(), mCommandList.Get(),
        flareTex->Filename.c_str(), flareTex->Resource, flareTex->UploadHeap);

    mTextures[flareTex->Name] = std::move(flareTex);

    auto flarealphaTex = std::make_unique<Texture>();
    flarealphaTex->Name = "flarealphaTex";
    flarealphaTex->Filename = L"../../ExerciseMedia/flarealpha.dds";
    CreateDDSTextureFromFile12(md3dDevice.Get(), mCommandList.Get(),
        flarealphaTex->Filename.c_str(), flarealphaTex->Resource, flarealphaTex->UploadHeap);
...
{
    mShaders["standardVS"] = d3dUtil::CompileShader(L"Shaders/Default_3.hlsl", nullptr, "VS", "vs_5_1");
    mShaders["opaquePS"] = d3dUtil::CompileShader(L"Shaders/Default_3.hlsl", nullptr, "PS", "ps_5_1");
    mInputLayout =
    {
