...
{
    auto woodCrate01Tex = std::make_unique<Texture>();
    woodCrate01Tex->Name = "woodCrate01Tex";
    woodCrate01Tex->Filename = L"../../Textures/WoodCrate01.dds";
    CreateDDSTextureFromFile12(md3dDevice.Get(), mCommandList.Get(),
        woodCrate01Tex->Filename.c_str(), woodCrate01Tex->Resource, woodCrate01Tex->UploadHeap);

    mTextures[woodCrate01Tex->Name] = std::move(woodCrate01Tex);

    auto woodCrate02Tex = std::make_unique<Texture>();
    woodCrate02Tex->Name = "woodCrate02Tex";
    woodCrate02Tex->Filename = L"../../Textures/WoodCrate02.dds";
    CreateDDSTextureFromFile12(md3dDevice.Get(), mCommandList.Get(),
        woodCrate02Tex->Filename.c_str(), woodCrate02Tex->Resource, woodCrate02Tex->UploadHeap);

    mTextures[woodCrate02Tex->Name] = std::move(woodCrate02Tex);
    ...
    CreateDDSTextureFromFile12(md3dDevice.Get(), mCommandList.Get(),
        checkboardTex->Filename.c_str(), checkboardTex->Resource, checkboardTex->UploadHeap);
...
void CrateApp::BuildDescriptorHeaps()
{
    ...
    auto woodCrate01Tex = mTextures["woodCrate01Tex"]->Resource;
    auto woodCrate02Tex = mTextures["woodCrate02Tex"]->Resource;
    ...
    srvDesc.Format = woodCrate01Tex->GetDesc().Format;
    srvDesc.Texture2D.MostDetailedMip = 0;
    srvDesc.Texture2D.MipLevels = -1;
    srvDesc.Texture2D.ResourceMinLODClamp = 0.0f;

    md3dDevice->CreateShaderResourceView(woodCrate01Tex.Get(), &srvDesc, hDescriptor);

    hDescriptor.Offset(1, mCbvSrvUavDescriptorSize);
    srvDesc.Format = woodCrate02Tex->GetDesc().Format;
    md3dDevice->CreateShaderResourceView(woodCrate02Tex.Get(), &srvDesc, hDescriptor);
...
void CrateApp::BuildShadersAndInputLayout()
{
    ...
    mInputLayout =
    {
        ...
        {"DIFFUSEMAPINDEX", 0, DXGI_FORMAT_R32_UINT, 0, 32, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0}
    };
}
...
void CrateApp::BuildBoxGeometry()
{
    ...
    std::vector<Vertex> vertices(box.Vertices.size() * 5);

    int vsize = box.Vertices.size();
    ...
    for (int i = 0; i < vsize; ++i)
    {
        ...
        vertices[i].diffuseMapIndex = 0;
    }

    XMFLOAT3 offset = { 1.0f, 0.0f, 1.0f };
    for (int i = vsize; i < 2 * vsize; ++i)
    {
        vertices[i].Pos = box.Vertices[i - vsize].Position + offset;
        vertices[i].Normal = box.Vertices[i - vsize].Normal;
        vertices[i].TexC = box.Vertices[i - vsize].TexC;
        vertices[i].diffuseMapIndex = 1;
    }
    
    offset = { 1.0f, 0.0f, -1.0f };
    for (int i = 2 * vsize; i < 3 * vsize; ++i)
    {
        vertices[i].Pos = box.Vertices[i - 2 * vsize].Position + offset;
        vertices[i].Normal = box.Vertices[i - 2 * vsize].Normal;
        vertices[i].TexC = box.Vertices[i - 2 * vsize].TexC;
        vertices[i].diffuseMapIndex = 2;
    }

    offset = { -1.0f, 0.0f, 1.0f };
    for (int i = 3 * vsize; i < 4 * vsize; ++i)
    {
        vertices[i].Pos = box.Vertices[i - 3 * vsize].Position + offset;
        vertices[i].Normal = box.Vertices[i - 3 * vsize].Normal;
        vertices[i].TexC = box.Vertices[i - 3 * vsize].TexC;
        vertices[i].diffuseMapIndex = 3;
    }

    offset = { -1.0f, 0.0f, -1.0f };
    for (int i = 4 * vsize; i < 5 * vsize; ++i)
    {
        vertices[i].Pos = box.Vertices[i - 4 * vsize].Position + offset;
        vertices[i].Normal = box.Vertices[i - 4 * vsize].Normal;
        vertices[i].TexC = box.Vertices[i - 4 * vsize].TexC;
        vertices[i].diffuseMapIndex = 4;
    }
    std::vector<std::uint16_t> indices(box.GetIndices16().size() * 5);
    int isize = box.GetIndices16().size();

    for (int i = 0; i < 5; i++)
        for (int j = 0; j < isize; j++)
            indices[i * isize + j] = vsize * i + box.GetIndices16()[j];
